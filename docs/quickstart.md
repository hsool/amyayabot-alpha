...
- **취득 방법**: [치지직 개발자 포털](https://developers.chzzk.naver.com/)에서 애플리케이션 등록
- **리디렉션 URL**: `http://localhost:8080/` 입력 (반드시 이 주소!)
- **권한 설정**: 등록 시 다음 권한을 모두 체크하세요:
  - 채팅 메시지 읽기 (필수)
  - 채팅 메시지 쓰기 (필수)
  - 후원 이벤트 수신 (필수)
  - 구독 이벤트 수신 (권장)
  - 채널 구독자 조회 (권장 — 구독자 전용 추첨 기능)
  - 방송 설정 변경 (선택 — 음성으로 방송 제목 변경 기능)
- 등록 완료 후 **Client ID**와 **Client Secret**을 복사해두세요
