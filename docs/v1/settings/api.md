...
**Step 1: 치지직 개발자 포털 접속**

1. [치지직 개발자 포털](https://developers.chzzk.naver.com/application)에 접속합니다
2. 네이버 계정으로 로그인합니다 (치지직 스트리머 계정과 동일해야 함)

**Step 2: 애플리케이션 등록**

1. 상단의 **"Application"** 메뉴를 클릭합니다
2. **"애플리케이션 등록"** 버튼을 클릭합니다
3. 다음 정보를 입력합니다:

   | 필드 | 입력 내용 |
   |------|----------|
   | **애플리케이션 ID** | 영문으로 자유롭게 입력 (예: `amyayabot`) |
   | **애플리케이션 이름** | 서비스 이름 (예: `AmyayaBot`) |
   | **로그인 리디렉션 URL** | `http://localhost:8080` |

   > ⚠️ **주의**: 애플리케이션 이름에 "chzzk", "치지직", "naver", "네이버" 등 공식 서비스명을 포함하면 등록이 거부됩니다.

   > ⚠️ **중요**: 리디렉션 URL은 반드시 **`http://`** 로 시작해야 합니다 (`https://`가 아님!). `https://`로 입력하면 AmyayaBot 인증이 실패합니다.

**Step 3: API 권한(스코프) 설정**

등록 화면에서 필요한 API 권한을 선택합니다. 다음 항목을 모두 체크해주세요:

   | 권한 (스코프) | 용도 | 필수 여부 |
   |------|------|---------|
   | **채팅 메시지 읽기** | 시청자 채팅을 읽고 AI가 반응 | 필수 |
   | **채팅 메시지 쓰기** | AI가 채팅에 메시지 전송 | 필수 |
   | **후원 이벤트 수신** | 후원 알림 감지 + AI 반응 | 필수 |
   | **구독 이벤트 수신** | 구독 알림 감지 + AI 반응 | 권장 |
   | **채널 구독자 조회** | 구독자 전용 추첨, 구독자 여부 확인 | 권장 |
   | **방송 설정 변경** | 음성 명령으로 방송 제목/카테고리 변경 | 선택 |

   > 💡 **팁**: 나중에 기능을 추가할 때 권한을 다시 신청하면 재승인이 필요합니다. 가능하면 처음에 모두 체크하는 것을 권장합니다.

4. **"저장"** 버튼을 클릭하여 등록 완료

**Step 4: Client ID / Secret Key 확인**

1. 등록한 애플리케이션을 클릭합니다
2. 애플리케이션 상세 페이지에서:
   - **Client ID** (긴 문자열)
   - **Secret Key** (암호처럼 보이는 문자열)

   이 두 값을 복사해두세요.

**Step 5: AmyayaBot에 입력**

1. AmyayaBot 설정 페이지(`http://localhost:18300/settings`) → "API 설정" 탭
2. **Client ID** 필드에 복사한 ID를 붙여넣기
3. **Client Secret** 필드에 복사한 Secret Key를 붙여넣기
4. 설정이 자동 저장됩니다
