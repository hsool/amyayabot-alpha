...
### 2. 치지직(네이버) 계정
치지직 방송 채팅에 연결하기 위해 필요합니다. 별도의 개발자 등록이나 API 키 발급은 필요 없습니다.

- 치지직 스트리머 계정과 동일한 네이버 계정으로 로그인만 하면 됩니다
...
OBS에 브라우저 소스 **2개**를 추가해야 합니다:

| 소스 | URL | 내용 |
|------|-----|------|
| 메인 오버레이 | `http://localhost:18300/overlay` | 아바타 + 말풍선 |
| 상호작용 오버레이 | `http://localhost:18300/overlay/interactive` | 투표, 추첨, 룰렛 UI |

아래 "OBS 연결" 섹션에서 추가 방법을 확인하세요.
...
1. **"연결하기"** 버튼 클릭
2. 브라우저에서 네이버 로그인 페이지가 열립니다 (정상입니다!)
3. 네이버 계정으로 로그인 후 권한 승인하면 자동 완료

> 💡 **처음 연결 시**: 인증 과정에서 `localhost:8080` 포트를 임시로 사용합니다. 다른 프로그램이 8080 포트를 사용하고 있으면 충돌할 수 있으니 확인하세요.
...
### 브라우저 소스 설정 — 소스 2: 상호작용 오버레이

투표, 추첨, 룰렛 등 게임 UI를 방송에 표시하려면 두 번째 브라우저 소스를 추가합니다.

1. **소스(Sources)** 창 아래 **+** 버튼 클릭 → **브라우저(Browser)** 선택
2. 이름은 "AmyayaBot 상호작용 오버레이" 같이 입력
3. **URL** 필드에 다음을 입력:
   ```
   http://localhost:18300/overlay/interactive
   ```
4. **너비**: 1920, **높이**: 1080
5. **확인** 클릭

이제 오버레이 2개가 OBS 미리보기에 표시됩니다!

> 💡 **레이어 순서**: 소스 목록에서 상호작용 오버레이를 메인 오버레이 위에 배치하면, 투표/추첨 화면이 아바타 앞에 표시됩니다.
